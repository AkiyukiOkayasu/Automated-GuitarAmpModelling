...
def main(args):
    ...
    if args.model == 'SimpleRNN':
        model_name = args.model + args.device + '_' + args.unit_type + \
            '_hs' + str(args.hidden_size) + '_pre_' + args.pre_filt
    ...
    save_path = os.path.join(
        args.save_location, args.device + '-' + args.load_config)
    ...
    optimiser = torch.optim.Adam(
        network.parameters(), lr=args.learn_rate, weight_decay=1e-4)
    ...
    for epoch in range(train_track['current_epoch'] + 1, args.epochs + 1):
        ...
        writer.add_scalar('Time/EpochTrainingTime',
                          time.time()-ep_st_time, epoch)
        ...
        if epoch % args.validation_f == 0:
            ...
            if val_loss < train_track['best_val_loss']:
                ...
                patience_counter += 1
            train_track.val_epoch_update(
                val_loss.item(), val_ep_st_time, time.time())
            writer.add_scalar('TrainingAndValidation/ValidationLoss',
                              train_track['validation_losses'][-1], epoch)

        print('current learning rate: ' + str(optimiser.param_groups[0]['lr']))
        train_track.train_epoch_update(
            epoch_loss.item(), ep_st_time, time.time(), init_time, epoch)
        # write loss to the tensorboard (just for recording purposes)
        writer.add_scalar('TrainingAndValidation/TrainingLoss',
                          train_track['training_losses'][-1], epoch)
        writer.add_scalar('TrainingAndValidation/LearningRate',
                          optimiser.param_groups[0]['lr'], epoch)
        network.save_model('model', save_path)
    ...
    test_loss_ESR = lossESR(
        test_output, dataset.subsets['test'].data['target'][0])
    test_loss_DC = lossDC(
        test_output, dataset.subsets['test'].data['target'][0])
...
if __name__ == "__main__":
    ...
    prsr.add_argument('--device', '-p', default='ht1',
                      help='This label describes what device is being modelled')
    prsr.add_argument('--data_location', '-dl', default='..',
    ...
    prsr.add_argument('--config_location', '-cl', default='Configs',
                      help='Location of the "Configs" directory')
    prsr.add_argument('--save_location', '-sloc', default='Results',
                      help='Directory where trained models will be saved')
    prsr.add_argument('--load_model', '-lm', default=True,
                      help='load a pretrained model if it is found')
    prsr.add_argument('--seed', default=None, type=int,
                      help='seed all of the random number generators if desired')

    # pre-processing of the training/val/test data
    prsr.add_argument('--segment_length', '-slen', type=int,
                      default=22050, help='Training audio segment length in samples')

    # number of epochs and validation
    prsr.add_argument('--epochs', '-eps', type=int, default=2000,
                      help='Max number of training epochs to run')
    prsr.add_argument('--validation_f', '-vfr', type=int,
                      default=2, help='Validation Frequency (in epochs)')
    # TO DO
    ...
    prsr.add_argument('--batch_size', '-bs', type=int,
                      default=50, help='Training mini-batch size')
    ...
    prsr.add_argument('--learn_rate', '-lr', type=float,
                      default=0.005, help='Initial learning rate')
    ...
    prsr.add_argument('--model', '-m', default='SimpleRNN',
                      type=str, help='model architecture')
    prsr.add_argument('--input_size', '-is', default=1, type=int,
                      help='1 for mono input data, 2 for stereo, etc ')
    prsr.add_argument('--output_size', '-os', default=1, type=int,
                      help='1 for mono output data, 2 for stereo, etc ')
    prsr.add_argument('--num_blocks', '-nb', default=1,
                      type=int, help='Number of recurrent blocks')
    prsr.add_argument('--hidden_size', '-hs', default=16,
                      type=int, help='Recurrent unit hidden state size')
    prsr.add_argument('--unit_type', '-ut', default='LSTM',
                      help='LSTM or GRU or RNN')
    prsr.add_argument('--skip_con', '-sc', default=1,
                      help='is there a skip connection for the input to the output')
